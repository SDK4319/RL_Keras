...
class DQNAgent:
    def __init__(self, action_size, state_size=(84, 84, 4)):
        ...
        self.model_path = os.path.join(os.getcwd(), 'save_model','breakout')
    ...
    def train_model(self):
        ...
        with tf.GradientTape() as tape:
            #Q Function about current state
            predicts = self.model(history)
            one_hot_action = tf.one_hot(actions, self.action_size)
            predicts = tf.reduce_sum(one_hot_action * predicts, axis=1)

            #Q Function about next state
    ...
    for e in range(num_episode):
        ...
        while not done:
            ...
            if dead:
                ...
                history = np.reshapce([history], (1, 84, 84, 4))
            else:
        ...
        if e % 1000 == 0:
            agent.model.save_weights("./save_model/breakout", save_format="tf")
